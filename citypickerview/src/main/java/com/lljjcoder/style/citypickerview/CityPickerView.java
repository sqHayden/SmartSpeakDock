...
public class CityPickerView implements CanShow, OnWheelChangedListener {
    ...
    public void setOnCityItemClickListener(OnCityItemClickListener listener) {mBaseListener = listener;}
    ...
    // 设置属性
    public void setConfig(final CityConfig config) {this.config = config;}
    ...
    private void initCityPickerPopwindow(Context context) {
        ...
        mOK = (Button) popview.findViewById(R.id.tv_confirm);
        mTvTitle = (TextView) popview.findViewById(R.id.tv_title);
        mCancel = (Button) popview.findViewById(R.id.tv_cancel);
...
//        popwindow.setFocusable(true);
        ...
        //标题
        if (!TextUtils.isEmpty(config.getTitle())) {mTvTitle.setText(config.getTitle());}
        
        //标题文字大小
        if (config.getTitleTextSize() > 0) {mTvTitle.setTextSize(config.getTitleTextSize());}
        ...
        if (!TextUtils.isEmpty(config.getConfirmText())) {mOK.setText(config.getConfirmText());}
        
        if ((config.getConfirmTextSize() > 0)) {mOK.setTextSize(config.getConfirmTextSize());}
        ...
        if (!TextUtils.isEmpty(config.getCancelText())) {mCancel.setText(config.getCancelText());}
        
        if ((config.getCancelTextSize() > 0)) {mCancel.setTextSize(config.getCancelTextSize());}
        ...
        setUpData();
        //背景半透明
        if (config != null && config.isShowBackground()) {utils.setBackgroundAlpha(context, 0.5f);}
    }
    
    // 加载数据
    private void setUpData() {
        if (parseHelper == null || config == null) {return;}
...
//        mViewProvince.setVisibleItems(config.getVisibleItems());
    ...
    private void updateCities() {
        ...
        if (parseHelper.getPro_CityMap() == null) {return;}
    ...
    public void showCityPicker(Context context) {
        ...
        if (!isShow()) {popwindow.showAtLocation(popview, Gravity.BOTTOM, 0, 0);}
    }
    
    @Override
    public void hide() {if (isShow()) {popwindow.dismiss();}}
